...
dict_get_goodsList = {'client':'e22c59101699ac978ee83bfb81d4c949','page':1,'per_page':5,'time':'1464340958'}
...
def get_sign(dict_get_goodsList,secret):
    #字典数据排序
    sourt_dict = sorted(dict_get_goodsList.items(),key=lambda d:d[0])
    #拼接字符串
    s=''
    for d in sourt_dict:
        s += str(d[0])
...
def new_sign():
    time.sleep(5)
    dict_get_goodsList['time'] = get_time()
    dict_get_goodsList['sign'] = get_sign(dict_get_goodsList,secret)
    return dict_get_goodsList
