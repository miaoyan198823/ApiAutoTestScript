...
dict_get_goods = {'client':'e22c59101699ac978ee83bfb81d4c949','time':'1464340958','goods_id':'7448806'}
...
def get_sign(dict_get_goods,secret):
    #字典数据排序
    sourt_dict = sorted(dict_get_goods.items(),key=lambda d:d[0])
    #拼接字符串
    s=''
    for d in sourt_dict:
        s += str(d[0])
...
def new_sign():
    dict_get_goods['time'] = get_time()
    dict_get_goods['sign'] = get_sign(dict_get_goods,secret)
    return dict_get_goods
