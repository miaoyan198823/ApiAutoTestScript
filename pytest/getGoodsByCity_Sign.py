...
dict_get_goodsByCity = {'client':'e22c59101699ac978ee83bfb81d4c949','city_id':'2419','time':'1464340958','page':1,'per_page':5}
...
def get_sign(dict_get_goodsByCity,secret):
    #字典数据排序
    sourt_dict = sorted(dict_get_goodsByCity.items(),key=lambda d:d[0])
    #拼接字符串
    s=''
    for d in sourt_dict:
        s += str(d[0])
...
def new_sign():
    dict_get_goodsByCity['time'] = get_time()
    dict_get_goodsByCity['sign'] = get_sign(dict_get_goodsByCity,secret)
    return dict_get_goodsByCity
