...
dict_get_goodsScore = {'client':'e22c59101699ac978ee83bfb81d4c949','goods_id':'8036538','time':'1464340958'}
...
def get_sign(dict_get_goodsScore,secret):
    #字典数据排序
    sourt_dict = sorted(dict_get_goodsScore.items(),key=lambda d:d[0])
    #拼接字符串
    s=''
    for d in sourt_dict:
        s += str(d[0])
...
def new_sign():
    dict_get_goodsScore['time'] = get_time()
    dict_get_goodsScore['sign'] = get_sign(dict_get_goodsScore,secret)
    return dict_get_goodsScore